+!~-(function(PIXI, window, document, undefined) {
    var waterfallCanvas = function(c, cw, ch) {
        ...
        this.particleRate = 6;
        this.gravity = 0.15;
        this.init = function() { this.loop(); };
        this.reset = function() { this.ctx.clearRect(0, 0, this.cw, this.ch);
            this.particles = []; };
        ...
        this.Particle.prototype.update = function(i) { this.vx += this.vx;
            this.vy += _this.gravity;
            this.x += this.vx;
            this.y += this.vy; };
        this.Particle.prototype.render = function() { _this.ctx.strokeStyle = 'hsla(' + this.hue + ', ' + this.saturation + '%, ' + this.lightness + '%, .05)';
            _this.ctx.beginPath();
            ...
            _this.ctx.stroke(); };
        this.Particle.prototype.renderBubble = function() { _this.ctx.fillStyle = 'hsla(' + this.hue + ', 40%, 40%, 1)';
        ...
        this.createParticles = function() {
            ...
            while (i--) { this.particles.push(new this.Particle()); } };
        this.removeParticles = function() {
            var i = this.particleRate;
            while (i--) {
                var p = this.particles[i];
                if (p.y > _this.ch - 20 - p.height) { p.renderBubble();
        ...
        this.renderParticles = function() {
            ...
            while (i--) {
                var p = this.particles[i];
                p.render(); } };
        this.clearCanvas = function() { this.ctx.globalCompositeOperation = 'destination-out';
            this.ctx.fillStyle = 'rgba(255,255,255,.06)';
            this.ctx.fillRect(0, 0, this.cw, this.ch);
            this.ctx.globalCompositeOperation = 'lighter'; };
        this.loop = function() {
            var loopIt = function() { requestAnimationFrame(loopIt, _this.c);
                _this.clearCanvas();
    ...
    if (isCanvasSupported()) {
        ...
        setupRAF();
        waterfall.init(); } /* Second plugin */
    var w, h, renderer, stage, waveGraphics, partGraphics, waveTexture, partTexture, waveCount, partCount, waves, parts;

    function init() { renderer = PIXI.autoDetectRenderer(window.innerWidth, window.innerHeight / 2, { backgroundColor: '0x' + tinycolor('hsl(200, 50%, 10%)').toHex() });
    ...
    function reset() { w = Math.max(document.getElementById('surface').scrollWidth, document.getElementById('surface').offsetWidth, document.getElementById('surface').clientWidth, document.getElementById('surface').scrollWidth, document.getElementById('surface').offsetWidth);
        h = Math.max(document.getElementById('surface').scrollHeight, document.getElementById('surface').offsetHeight, document.getElementById('surface').clientHeight, document.getElementById('surface').scrollHeight, document.getElementById('surface').offsetHeight);
        renderer.resize(w, h);
        waveGraphics = null;
        waveTexture = null;
        partGraphics = null;
    ...
    function step() {
        ...
        for (var i = 0, length = waves.length; i < length; i++) {
            ...
            wave.y += wave.vy;
            wave.speed *= 1.01;
            if (wave.x > w + 200 || wave.x < -200 || wave.y > h + 200) { wave.x = w / 2;
                wave.y = h / 2;
        ...
        for (var i = 0, length = parts.length; i < length; i++) {
            ...
            part.y += part.vy;
            part.speed *= 1.01;
            if (part.x > w + 50 || part.x < -50 || part.y < -50) { part.x = part.ox;
                part.y = part.oy;
